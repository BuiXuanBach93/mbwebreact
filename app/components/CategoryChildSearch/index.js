...
class CategoryChildSearch extends Component {
    constructor(props) {
        super(props);
        this.categories = this.props.categories;
        this.state = {categories:[], category:{categoryId: 0, categoryName: 'すべて'}};

        // console.log("component child : ", this.categories);
    }
...
render() {
    if(!this.categories){
        ...
        const categoriesElement = this.props.categories.map((item, i) => {
            return <Li key={item.categoryId}>
                <ChooseDiv onClick={()=> {this.props.handleClickChildCategory(item.categoryId, item.categoryName);}}>
                        <ChosenLabelDiv>
...
export default CategoryChildSearch;